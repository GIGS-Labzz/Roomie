...
import { Check, X, Shield, Settings, AlertCircle } from "lucide-react";
...
export function CookieBanner() {
  ...
  const saveConsent = (updatedSettings: CookieSettings) => {
    ...
    // Set custom event so other components can listen to cookie consent change if needed
    window.dispatchEvent(new CustomEvent("cookie-consent-changed", { detail: updatedSettings }));
    
    // If analytics is disabled, we can configure clean-up of Google Analytics here
    if (!updatedSettings.analytics) {
      // Clear GA cookies or disable GA tracking
      try {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed bottom-4 left-4 right-4 z-50 mx-auto max-w-lg rounded-3xl border border-slate-200/50 bg-white/90 p-6 shadow-[0_15px_50px_rgba(0,0,0,0.12)] backdrop-blur-xl md:left-auto md:right-6 md:w-[440px]"
        >
