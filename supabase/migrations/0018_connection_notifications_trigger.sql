-- Create a database trigger to insert notifications automatically on connection inserts/updates.
-- This bypasses any client-side RLS policy restrictions on the notifications table.
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- If a new connection is created (starts in PENDING_PAYMENT status), notify the receiver.
...
CREATE TRIGGER on_connection_change
  ...
  FOR EACH ROW EXECUTE FUNCTION public.notify_on_connection_change();
